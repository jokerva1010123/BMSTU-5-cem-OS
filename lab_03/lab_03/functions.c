...
void print_inform(const description *const items_array, const int items_len, int argc, char *filter)
{
    for (int i = 0; i < items_len; i++)
    {
        ...
        {
            if (strncmp(filter, items_array[i].title, strlen(filter)) == OK)
                print_structure(items_array, i);
        }
    }
}
...
void insertion_sort(description *items_array, const int items_len)
{
    for (int i = 1; i < items_len; i++)
    {
        ...
        int j = i - 1;
        float start_el = items_array[i].weight / items_array[i].volume;
        for (; items_array[j].weight / items_array[j].volume > start_el && j >= 0;)
        {
